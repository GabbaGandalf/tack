resource "aws_launch_configuration" "worker-gpu" {
  ebs_block_device {
...
resource "aws_autoscaling_group" "worker-gpu" {
  name = "worker-gpu-${ var.worker-name }-${ var.name }"

  desired_capacity = "${ var.capacity["desired"] }"
  health_check_grace_period = 60
  health_check_type = "EC2"
  force_delete = true
  launch_configuration = "${ aws_launch_configuration.worker-gpu.name }"
  #TODO variable for gpu cap 
